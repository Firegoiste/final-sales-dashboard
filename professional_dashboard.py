# professional_dashboard.py (云端就绪最终版)
...
# --- 核心修改：使用您指定的原始数据文件名 ---
SOURCE_DATA_FILE = 'sales_data.xlsx'
...
def load_data_from_excel(file_path):
    ...
    try:
        ...
        st.error(f"错误：在仓库中找不到数据文件 '{file_path}'。请确保您已将此文件上传到GitHub。")
        return pd.DataFrame()
...
# 从指定的Excel文件加载所有数据
df_all = load_data_from_excel(SOURCE_DATA_FILE)

# (后续的所有代码，包括 NLU, KPI, 图表, 预测等，和我们之前的最终版完全一样，无需任何改动)
# --- 辅助函数：导出Excel ---
@st.cache_data
def to_excel(df):
    output = BytesIO()
...
def parse_query(query, data_df):
    ...
    if found_rep and not found_region and not found_category:
        ...
        product_text = "\n#### **Top 3 畅销产品:**\n"
        for i, (product, sales) in enumerate(top_products.items(), 1): product_text += f"**{i}. {product}:** ¥ {sales:,.2f}\n"
    ...
    if found_region:
        original_regions = [region for region in data_df['销售区域 (Region)'].unique() if str(region).lower() in found_region]
...
selected_date = st.sidebar.date_input("选择查看日期", value=df_all['日期 (Date)'].max(), min_value=df_all['日期 (Date)'].min(), max_value=df_all['日期 (Date)'].max())
...
df_selected = df_selected_day_unfiltered[(df_selected_day_unfiltered['销售代表 (Rep)'].isin(selected_reps)) & (df_selected_day_unfiltered['产品大类 (Category)'].isin(selected_categories))]
...
if df_selected.empty:
    ...
    daily_growth_rate = ((total_sales_selected - total_sales_previous) / total_sales_previous) * 100 if total_sales_previous > 0 else 0
    total_sales_last_week = df_last_week['销售额 (Sales)'].sum()
    weekly_growth_rate = ((total_sales_selected - total_sales_last_week) / total_sales_last_week) * 100 if total_sales_last_week > 0 else 0
    ...
    st.download_button(label="📄 下载当日筛选后明细数据", data=excel_data, file_name=f"sales_details_{selected_date.strftime('%Y%m%d')}.xlsx")
    st.markdown("---")
    tab1, tab2, tab3 = st.tabs(["📊 **图表联动分析 (筛选后)**", "📈 历史趋势 (全量)", "🔮 销售预测 (全量)"])
    with tab1:
        st.subheader("各维度销售额详情 (点击图表进行联动筛选)")
        selection = alt.selection_multi(fields=['销售区域 (Region)'], empty='all')
        chart_region = alt.Chart(df_selected).mark_bar().encode(
            x=alt.X('销售区域 (Region):N', title='销售区域'),
    ...
    with tab3:
        ...
        if len(prophet_df) < 14:
            ...
            display_forecast.rename(columns={'ds': '日期', 'yhat': '预测值', 'yhat_lower': '预测下限 (95%置信)', 'yhat_upper': '预测上限 (95%置信)'}, inplace=True)
